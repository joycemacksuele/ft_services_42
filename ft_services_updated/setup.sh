...
if [[ $1 = 'delete' || $1 = 'correction' || $1 = '' ]] ; then
	sleep 1
else
	echo -e "\n$bold_red Only accepted arguments:$bold_white delete or correction$reset\n"
	exit
fi
...
do
	...
	if [ $? -ne 0 ]
	then
		echo -e "$bold_redERROR: service $service is missing or was not named correctly$reset"
		exit
	else
...
do
	...
	then
		...
		echo -e "$bold_redERROR: $service is not running$reset"
		exit
	fi
done
...
for service in $SERVICES
do
	POD_NAME=`kubectl get pods | grep $Sservice | cut -d ' ' -f 1`
	case $service in
		nginx)
		...
		then
			...
			echo -e "$bold_red SSH not killedi$reset"
		fi
		sleep 5
		...
		if [ $? -ne 1 ]
		then
			echo -e "$bold_green $service killedi$reset"
		else
...
sleep 10
echo -e "\n- Check in another terminal if there are any pods that can't restart:"
echo -e " Command:$bold_green kubectl get pods$reset\n"
exit
fi
...
\clear
	...
	echo -e "    - with redirect to https:     $link_cyan http://$MINIKUBE_IP$reset or$link_cyan $MINIKUBE_IP$reset"
	echo -e "    - https:                      $link_cyan https://$MINIKUBE_IP$reset"
	echo -e "    - reverse proxy to phpma:     $link_cyan https://$MINIKUBE_IP/phpmyadmin$reset"
	echo -e "    - temporary redirect to wp:   $link_cyan https://$MINIKUBE_IP/wordpress$reset"
	echo -e "    - ssh:                        $>$link_cyan ssh -o StrictHostKeyChecking=no user@192.168.49.3 -p 22$reset"
...
echo -e "\n$bold_white Press 'ENTER' to start the Kubernetes web dashboard$link_cyan\n"
read REPLY
