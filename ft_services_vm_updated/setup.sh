...
########################### Correction ###########################
	...
	for service in $SERVICES
	do
		kubectl get pods | grep $service- &> /dev/null
		if [ $? -ne 0 ]
		then
	...
	do
		...
		case $service in
			nginx)
			kubectl exec $POD_NAME -- pkill sshd &> /dev/null
			if [ $? -ne 1 ]
			then
			...
			sleep 5
			kubectl exec $POD_NAME -- pkill nginx &> /dev/null
			if [ $? -ne 1 ]
			then
			...
			;;
			telegraf)
			kubectl exec $POD_NAME -- pkill telegraf &> /dev/null
			if [ $? -ne 1 ]
			then
				echo -e "$bold_green $service killed$reset"
			else
			...
			;;
			ftps)
			kubectl exec $POD_NAME -- pkill vsftpd &> /dev/null
			if [ $? -ne 1 ]
			then
				echo -e "$bold_green $service killed$reset"
			else
			...
			;;
			mysql)
			kubectl exec $POD_NAME -- pkill mysqld &> /dev/null
			sleep 5
			kubectl exec $POD_NAME -- pkill mysqld_safe &> /dev/null
			sleep 5
			kubectl exec $POD_NAME -- pkill mariadb &> /dev/null
			if [ $? -ne 1 ]
			then
				echo -e "$bold_green $service killed$reset"
			else
			...
			;;
			influxdb)
			kubectl exec $POD_NAME -- pkill influxd &> /dev/null
			if [ $? -ne 1 ]
			then
				echo -e "$bold_green $service killed$reset"
			else
			...
			;;
			grafana)
			kubectl exec $POD_NAME -- pkill grafana &> /dev/null
			if [ $? -ne 1 ]
			then
				echo -e "$bold_green $service killed$reset"
			else
			...
			;;
			wordpress)
			kubectl exec $POD_NAME -- pkill php-fpm &> /dev/null
			sleep 5
			kubectl exec $POD_NAME -- pkill nginx &> /dev/null
			if [ $? -ne 1 ]
			then
